...
<style>
    ...
    <body>
        <nav class="navbar navbar-fixed-top androidi" style="background-color:#6ab344 ">
            <div class="navbar-header">
                <a class="navbar-brand" href="Home.php">&lt;C<b style="color: red;">&#x2764;</b>C&gt;</a>
            </div>
            <h1 class="text-center"><b>ANDROID</b></h1>    
        </nav>
        <div class="container">
            <div class="row top">
                <div class="card smartphone" style="border-color:#6ab344 ">
                    <iframe  class="col utvideo" style="background-color: white;" src="https://www.youtube.com/embed/Yo6Y_piTa1I?rel=0&autoplay=1&controls=1&showinfo=0&autohide=1&loop=1"  frameborder="0" allowfullscreen></iframe>
                </div>
            </div>
        </div>
        ...
        <div class="container">
			...
			echo"<div class='w3-row'>
            	...
            	echo"<div class='w3-row'>";
				$sql = "select id,documentations from android order by id asc"; 
				$result = $conn->query($sql);
				if ($result->num_rows > 0) {
					while($row = $result->fetch_assoc()) {
        				if(!$row['documentations']== null)
						{
							echo "<div class='w3-col' style='width:inherit'><div class='card android foo'> ";
...
<div class='heartbutton' data-post_id='102' data-post_animation='season1' value='".$row['id']."' name='documentations'>
      	...
      	<img class='heart-icon-animation heartsize' src='http://machineboy.com/bilder/heart-icon-explode.svg'/>
			...
			echo "<div class='w3-row'>
            	...
            	echo"<div class='w3-row'>";
				$sql = "select id,courses from android order by id asc"; 
				$result = $conn->query($sql);
				if ($result->num_rows > 0) {
					while($row = $result->fetch_assoc()) {
        				if(!$row['courses']== null)
	...
	<div class='heartbutton' data-post_id='102' data-post_animation='season1' value='".$row['id']."' name='courses'>
      	...
      	<img class='heart-icon-animation heartsize' src='http://machineboy.com/bilder/heart-icon-explode.svg'/>
			...
			echo "<div class='w3-row'>
            	...
            	echo "<div class='w3-row'>";
				$sql = "select id,apps from android order by id asc"; 
				$result = $conn->query($sql);
				if ($result->num_rows > 0) {
					while($row = $result->fetch_assoc()) {
        				if(!$row['apps']== null)
	...
	<div class='heartbutton' data-post_id='102' data-post_animation='season1' value='".$row['id']."' name='apps'>
        ...
        <h1 class="ft"><b>Let's make the android more sweeter than ever !!</b></h1> 
        <img class="mid bottom img-thumbnail" style="background-color: #9cec9d;" src="https://www.android.com/static/2016/img/home/holiday-hero/android-8-oreo_m.gif">
