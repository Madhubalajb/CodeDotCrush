...
<html lang="en">
<head>
    <title>Explore Networking</title>
...
<style>
    ...
    <body>
        <nav class="navbar navbar-fixed-top neti" style="background-color:#590D82;">
            <div class="navbar-header">
                <a class="navbar-brand" href="Home.php">&lt;C<b style="color: red;">&#x2764;</b>C&gt;</a>
            </div> 
            <h1 class="text-center"><b>NETWORKING</b></h1>  
        </nav>
        <div class="container">
            <div class="row top">
                <div class="card smartphone" style="border-color:#590D82; ">
                    <iframe class="col utvideo" src="https://www.youtube.com/embed/Dxcc6ycZ73M?autoplay=1&rel=0&controls=0&showinfo=0" frameborder="0" allowfullscreen></iframe>
                </div>
            </div>
        </div>
        
        <div class="videoWrapper foo"  style="background-image: url(Images/cn1.jpg)">
        </div> 
        ...
        <div class="container">
			...
			echo"<div class='w3-row'>
            	...
            	echo"<div class='w3-row'>";
				$sql = "select id,documentations from networking order by id asc"; 
				$result = $conn->query($sql);
				if ($result->num_rows > 0) {
					while($row = $result->fetch_assoc()) {
        				if(!$row['documentations']== null)
						{
							echo "<div class='w3-col' style='width:inherit'><div class='card net foo'> ";
...
<div class='heartbutton' data-post_id='102' data-post_animation='season1' value='".$row['id']."' name='documentations'>
	...
	<img class='heart-icon-animation heartsize' src='http://machineboy.com/bilder/heart-icon-explode.svg'/>
			...
			echo "<div class='w3-row'>
            	...
            	echo"<div class='w3-row'>";
				$sql = "select id,courses from networking order by id asc"; 
				$result = $conn->query($sql);
				if ($result->num_rows > 0) {
					while($row = $result->fetch_assoc()) {
        				if(!$row['courses']== null)
...
<div class='heartbutton' data-post_id='102' data-post_animation='season1' value='".$row['id']."' name='courses'>
	...
	<img class='heart-icon-animation heartsize' src='http://machineboy.com/bilder/heart-icon-explode.svg'/>
			...
			echo "<div class='w3-row'>
            	...
            	echo"<div class='w3-row'>";
				$sql = "select id,youtube from networking order by id asc"; 
				$result = $conn->query($sql);
				if ($result->num_rows > 0) {
					while($row = $result->fetch_assoc()) {
        				if(!$row['youtube']== null)
...
<div class='heartbutton' data-post_id='102' data-post_animation='season1' value='".$row['id']."' name='youtube'>
	...
	<img class='heart-icon-animation heartsize' src='http://machineboy.com/bilder/heart-icon-explode.svg'/>
			...
			echo "<div class='w3-row'>
            	...
            	echo "<div class='w3-row'>";
				$sql = "select id,apps from networking order by id asc"; 
				$result = $conn->query($sql);
				if ($result->num_rows > 0) {
					while($row = $result->fetch_assoc()) {
        				if(!$row['apps']== null)
...
<div class='heartbutton' data-post_id='102' data-post_animation='season1' value='".$row['id']."' name='apps'>
	...
	<img class='heart-icon-animation heartsize' src='http://machineboy.com/bilder/heart-icon-explode.svg'/>
        ...
        <div class="container">
            <h1 class="ft"><b>Let's improve the connection .</b></h1>
            <img class="img-thumbnail mid" src="http://gifimage.net/wp-content/uploads/2017/10/networking-gif-10.gif">
        </div>
