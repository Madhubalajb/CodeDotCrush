...
<style>
    ...
    <body>
        <nav class="navbar navbar-fixed-top ioti" style="background-color:red;">
            <div class="navbar-header">
                <a class="navbar-brand" href="Home.php">&lt;C<b style="color: white;">&#x2764;</b>C&gt;</a>
            </div>
            <h1 class="text-center"><b>IOT</b></h1>    
        </nav>
        <div class="container">
            <div class="row top">
                <div class="card smartphone" style="border-color:red; ">
                    <iframe class="col utvideo" src="https://www.youtube.com/embed/p_R5ZVWMhzM?rel=0&autoplay=1&controls=1&showinfo=0&autohide=0" frameborder="0" allowfullscreen></iframe>
                </div>
            </div>
        </div>
        ...
        <div class="container">
			...
			echo"<div class='w3-row'>
            	...
            	echo"<div class='w3-row'>";
				$sql = "select id,documentations from iot order by id asc"; 
				$result = $conn->query($sql);
				if ($result->num_rows > 0) {
					while($row = $result->fetch_assoc()) {
        				if(!$row['documentations']== null)
						{
							echo "<div class='w3-col' style='width:inherit'><div class='card iot foo'> ";
...
<div class='heartbutton' data-post_id='102' data-post_animation='season1' value='".$row['id']."' name='documentations'>
	...
	<img class='heart-icon-animation heartsize' src='http://machineboy.com/bilder/heart-icon-explode.svg'/>
			...
			echo "<div class='w3-row'>
            	...
            	echo"<div class='w3-row'>";
				$sql = "select id,courses from iot order by id asc"; 
				$result = $conn->query($sql);
				if ($result->num_rows > 0) {
					while($row = $result->fetch_assoc()) {
        				if(!$row['courses']== null)
...
<div class='heartbutton' data-post_id='102' data-post_animation='season1' value='".$row['id']."' name='courses'>
	...
	<img class='heart-icon-animation heartsize' src='http://machineboy.com/bilder/heart-icon-explode.svg'/>
			...
			echo "<div class='w3-row'>
            	...
            	echo"<div class='w3-row'>";
				$sql = "select id,youtube from iot order by id asc"; 
				$result = $conn->query($sql);
				if ($result->num_rows > 0) {
					while($row = $result->fetch_assoc()) {
        				if(!$row['youtube']== null)
...
<div class='heartbutton' data-post_id='102' data-post_animation='season1' value='".$row['id']."' name='youtube'>
	...
	<img class='heart-icon-animation heartsize' src='http://machineboy.com/bilder/heart-icon-explode.svg'/>
			...
			echo "<div class='w3-row'>
            	...
            	echo "<div class='w3-row'>";
				$sql = "select id,apps from iot order by id asc"; 
				$result = $conn->query($sql);
				if ($result->num_rows > 0) {
					while($row = $result->fetch_assoc()) {
        				if(!$row['apps']== null)
...
<div class='heartbutton' data-post_id='102' data-post_animation='season1' value='".$row['id']."' name='apps'>
	...
	<img class='heart-icon-animation heartsize' src='http://machineboy.com/bilder/heart-icon-explode.svg'/>
        ...
        <img class="mid bottom img-thumbnail" src="Images/IOT1.jpg">
