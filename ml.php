...
<html lang="en">
<head>
    <title>Explore Machine Learning</title>
...
<style>
    ...
    <body>
        <nav class="navbar navbar-fixed-top" style="background-color:black;">
            ...
            <h1 class="text-center"><b>MACHINE LEARNING</b></h1>    
        </nav> 
        <div class="container">
            <div class="row top">
                <div class="card smartphone" style="border-color:black; ">
                    <iframe class="col utvideo" src="https://www.youtube.com/embed/f_uwKZIAeM0?rel=0&autoplay=1&controls=1&showinfo=0&autohide=0" frameborder="0" allowfullscreen></iframe>
                </div>
            </div>
        </div>
        ...
        <div class="container">
			...
			echo"<div class='w3-row'>
            	...
            	echo"<div class='w3-row'>";
				$sql = "select id,documentations from machine_learning order by id asc"; 
				$result = $conn->query($sql);
				if ($result->num_rows > 0) {
					while($row = $result->fetch_assoc()) {
        				if(!$row['documentations']== null)
						{
							echo "<div class='w3-col' style='width:inherit'><div class='card ml foo'> ";
...
<div class='heartbutton' data-post_id='102' data-post_animation='season1' value='".$row['id']."' name='documentations'>
	...
	<img class='heart-icon-animation heartsize' src='http://machineboy.com/bilder/heart-icon-explode.svg'/>
			...
			echo "<div class='w3-row'>
            	...
            	echo"<div class='w3-row'>";
				$sql = "select id,courses from machine_learning order by id asc"; 
				$result = $conn->query($sql);
				if ($result->num_rows > 0) {
					while($row = $result->fetch_assoc()) {
        				if(!$row['courses']== null)
...
<div class='heartbutton' data-post_id='102' data-post_animation='season1' value='".$row['id']."' name='courses'>
	...
	<img class='heart-icon-animation heartsize' src='http://machineboy.com/bilder/heart-icon-explode.svg'/>
			...
			echo "<div class='w3-row'>
            	...
            	echo"<div class='w3-row'>";
				$sql = "select id,youtube from machine_learning order by id asc"; 
				$result = $conn->query($sql);
				if ($result->num_rows > 0) {
					while($row = $result->fetch_assoc()) {
        				if(!$row['youtube']== null)
...
<div class='heartbutton' data-post_id='102' data-post_animation='season1' value='".$row['id']."' name='youtube'>
	...
	<img class='heart-icon-animation heartsize' src='http://machineboy.com/bilder/heart-icon-explode.svg'/>
            ...
            <div class="container">
                <div class="jumbotron ml ft">
                    <p><strong style="word-spacing: 5px;">"A breakthrough in machine learning would be worth 10 Microsofts" - Bill Gates, Chairman, Microsoft </strong></p>
                </div>
            </div>
